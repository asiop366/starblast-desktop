...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$b='%SCRIPTS_BASE%'; $files=@('mod-sync.js','find-launcher-install.js','patch-starblast-launcher.js','update-launcher-mods.js'); foreach($f in $files){ try { Invoke-WebRequest -Uri ($b+'/'+$f) -OutFile (Join-Path 'scripts' $f) -UseBasicParsing; Write-Host ('  OK '+$f) } catch { Write-Host ('  WARN '+$f+' (copie locale)') } }"
if errorlevel 1 (
