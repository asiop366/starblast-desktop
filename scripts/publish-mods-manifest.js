...
const version = process.env.SB_MODS_VERSION || pkg.version;

execSync('node scripts/build-mods-bundle.js', { cwd: ROOT, stdio: 'inherit', env: Object.assign({}, process.env, { SB_MODS_VERSION: version }) });
execSync('node scripts/build-mods-bundle.js --launcher', { cwd: ROOT, stdio: 'inherit', env: Object.assign({}, process.env, { SB_MODS_VERSION: version }) });
...
if (!fs.existsSync(launcherBundleSrc)) {
  throw new Error('Missing launcher bundle after build');
}
const launcherBundle = fs.readFileSync(launcherBundleSrc, 'utf8');
if (launcherBundle.indexOf('window.__SB_DESKTOP_MODS_VERSION = ' + JSON.stringify(version)) < 0) {
  throw new Error('Launcher bundle version marker mismatch for v' + version);
}
if (launcherBundle.indexOf('hookSettingsPersistence') < 0) {
  throw new Error('Launcher bundle missing settings persistence hooks');
}
if (launcherBundle.indexOf('sb-color-row') < 0) {
  throw new Error('Launcher bundle missing settings color UI fix');
}
